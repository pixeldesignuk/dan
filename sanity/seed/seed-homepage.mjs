/**
 * Standalone seed script for the homepage document
 * Run with: bun sanity/seed/seed-homepage.mjs
 *
 * Requires SANITY_API_WRITE_TOKEN environment variable to be set
 */
...
if (!token) {
  ...
  console.error("Create a write token at: https://www.sanity.io/manage/project/" + projectId + "/api#tokens");
  process.exit(1);
}
...
async function seedHomepage() {
  console.log(`Seeding homepage document to ${projectId}/${dataset}...`);
  ...
  try {
    ...
    if (existing) {
      console.log("Homepage document already exists. Replacing...");
